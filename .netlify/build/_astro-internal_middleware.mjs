...
import 'html-escaper';
import 'clsx';
import './chunks/astro/server_-WQNH1EB.mjs';
import { timingSafeEqual, randomUUID, createHash } from 'node:crypto';
import { A as AstroError, R as ResponseSentError } from './chunks/astro/assets-service_DDgrU5qf.mjs';
...
function createSessionCookie(sessionId, secure = true) {
...
function recordLoginAttempt(request, success) {
...
function authenticatedSessionCookie(request, secure = true) {
...
function parseCookies(header) {
  return new Map(
    header.split(";").map((cookie) => cookie.trim()).filter(Boolean).map((cookie) => {
      const [name, ...rest] = cookie.split("=");
      return [name, rest.join("=")];
    })
  );
}
function getAttemptsForRequest(request) {
  const key = getRequestKey(request);
  const now = Date.now();
  const attempts = loginAttempts.get(key) || [];
  const recentAttempts = attempts.filter((timestamp) => now - timestamp < RATE_LIMIT_WINDOW_MS);
